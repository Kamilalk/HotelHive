...
import { getFirestore, collection, onSnapshot, addDoc, Timestamp, deleteDoc, doc, updateDoc, query, where, setDoc, getDoc } from 'firebase/firestore';
...
const MyTasks = () => {
  ...
  const addTaskNotification = async (task, id) => {
    const notification = {
      ...
      info: { due: task.due, taskAssignee: task.assignee}
    };
    try {
    ...
    try {
      ...
      if (updatedFields.assignee !== currentAssigneeId) {
        ...
        const newAssigneeNotification = {
          ...
          time: new Date().toLocaleTimeString(),
          date: new Date().toLocaleDateString(),
          info: { due: updatedTask.due, taskAssignee: updatedTask.assignee }
        };
