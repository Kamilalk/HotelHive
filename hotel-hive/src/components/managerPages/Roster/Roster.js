...
const Roster = () => {
  ...
  useEffect(() => {
    ...
    let unsubscribe = []; // Define unsubscribe outside the loop
    for (let i = 0; i < 7; i++) {
      ...
      unsubscribe.push(onSnapshot(currentShiftsRef, (querySnapshot) => {
        ...
        console.log(`Shifts for ${currentFormattedDate}:`, shiftsData); // Log the returned data
  
        // Update shifts state accordingly for each day
    ...
    return () => {
      // Call each unsubscribe function to unsubscribe from snapshots
  ...
  const [newShift, setNewShift] = useState({
    name: "", // Change to name as it holds the user ID
    date: "",
    startTime: "",
    endTime: ""
  });
  ...
  const handleAddShift = async () => {
    ...
    // Fetch user's details based on the selected name (which is the user ID)
        ...
        <div className="col-lg-3 col-md-6 col-sm-12 roster-form">
          ...
          <div className="roster-form-container">
            ...
            <input
              ...
              className="block w-full input-field"
            />
            <button className="submit-button" onClick={handleAddShift}>Add</button>
          </div>
        </div>
