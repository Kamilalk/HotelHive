...
const EditChat = ( ) => {
  ...
  const handleCheckboxChange = (id) => {
    if (selectedStaff.includes(id)) {
      // User is in the chat, so remove them
      setSelectedStaff(selectedStaff.filter(userId => userId !== id));
    } else {
      // User is not in the chat, so add them
  ...
  const handleDeleteChat = async () => {
    try {
      ...
      // You can also redirect the user to another page or show a success message after deleting the chat
    } catch (error) {
  ...
  return (
    <div className='col-lg-6 col-md-6 col-sm-12 edit-chat-input'>
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="form-btn">Update Chat</button>
        <button type="button" className="form-btn" onClick={handleDeleteChat}>Delete Chat</button>
      </form>
      {showSuccessPopup && (
