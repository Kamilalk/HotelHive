...
const CreateChat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const chatDocId = docRef.id;
      const chatUsersRef = collection(database, 'Hotels', hotelId, 'Chats', chatDocId, 'chatUsers');
      selectedStaff.forEach(async (staffId) => {
        await addDoc(chatUsersRef, { userId: staffId });
      });
      console.log('Chat created successfully:', chatName, selectedStaff);
